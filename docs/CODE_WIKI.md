...
- v1.6.1 完成跨进程 mutation 锁串行化、TypeScript 边界收紧与 Windows 并发覆盖扩展
...
v1.6.1 把所有持久化状态变更收敛到统一的 `withFileLockSync` / `mutateLockedJsonFileSync` 抽象：

| 状态域 | 锁文件位置 | 串行化操作 |
| --- | --- | --- |
| Task status | `tasks/<id>/status.lock` | `pending → running` 领取、cancel、状态原子替换 |
| Watcher | `.patchwarden/watcher.lock` | 单实例排他创建 + stale PID 接管 |
| Goal status | `.patchwarden/goals/<id>/goal.lock` | mutation、子目标 accept/reject |
| Worktree | `.patchwarden/worktrees/<id>/lifecycle.lock` | create/merge/discard 仓库级锁 |
| Direct session | `direct-sessions/<id>/session.lock` | patch/sync/verify/finalize/audit |
| Assessment | `assessments/<id>/assessment.lock` | freshness 校验与读取 |
| Control events | `.patchwarden/control-center/events.lock` | append + trim + 原子替换 |
| Bounded logs | `invocation.log.lock` 等 | 跨进程有界追加 + 截断标记 |

锁文件使用 `O_EXCL` 创建 + PID 探活，孤儿锁由后续操作自动接管；所有锁内操作完成后通过原子 `.tmp-{pid}` rename 提交，保证 crash 后磁盘上要么是旧状态、要么是新状态，不会出现半写入。
...
│   │   ├── discoveryTokenStore.ts  workspaceRootGuard.ts  loopbackHost.ts
│   ├── tools/                    # MCP 工具实现
│   │   ├── registry.ts           # 工具注册中枢
│   │   ├── catalog/              # Profile / Manifest / 搜索 / 统计
│   │   │   ├── toolCatalog.ts  toolRegistry.ts  toolSearch.ts  toolUsageStats.ts
│   │   ├── definitions/          # 工具定义
│   │   │   └── toolDefs.ts
│   │   ├── diagnostics/          # 审计与健康检查
│   │   │   ├── auditSession.ts  auditTask.ts  healthCheck.ts
│   │   │   ├── safeStatus.ts  safeViews.ts  schemaDriftCheck.ts
│   │   ├── direct/               # Direct 工具实现
│   │   ├── discovery/            # 动态工具发现
│   │   ├── dispatch/             # 工具分派层（v1.5.1 拆分）
│   │   │   ├── types.ts  coreDispatch.ts  diagnosticDispatch.ts
│   │   │   ├── directDispatch.ts  goalDispatch.ts  releaseDispatch.ts
│   │   │   └── validation.ts
│   │   ├── goals/                # Goal 工具
│   │   ├── release/              # Release mode 工具
│   │   ├── tasks/                # 任务管理工具
│   │   ├── workspace/            # 工作区与 worktree 工具
│   │   ├── registry.ts
│   │   └── taskTemplates.ts
...
│   ├── src/                      # 9 个 .ts 模块
│   │   ├── main.ts               # Electron 入口、Tray、IPC、后端生命周期
│   │   ├── agent-adapters.ts     # 8 种 Agent 适配器
│   │   ├── agent-detection.ts    # 门面 re-export
│   │   ├── backend-probe.ts      # /api/diagnostics 探测
│   │   ├── config-store.ts       # 配置/DPAPI/原子写入
│   │   ├── model-discovery.ts    # 各 Agent 配置文件模型抽取
│   │   ├── runtime-root.ts       # 主仓定位
│   │   ├── runtime-settings.ts   # tunnel-client 检测
│   │   └── tunnel-provisioner.ts # 隧道下发
│   ├── onboarding/               # 首次引导 UI
...
| [sensitiveGuard.ts](../src/security/sensitiveGuard.ts) | 敏感文件守卫：`isSensitivePath` 匹配 `.env`/SSH 私钥/`credentials`/`.npmrc`/`cookies`/`.kube/config` 等；没有 `.patchwarden/` 前缀豁免，敏感名称在任意目录深度都阻断；NTFS ADS、NUL 字节也拒绝 |
...
| [runtimeGuard.ts](../src/security/runtimeGuard.ts) | 阻止任务把 `repo_path` 指向运行中的 PatchWarden 自身目录（`dist`/`src`/`scripts`/`release`）；使用 `isSamePath`/`isPathChildOf` 做平台感知比较 |
| [contentRedaction.ts](../src/security/contentRedaction.ts) | 输出脱敏：`redactSensitiveContent`/`redactSensitiveValue` 按正则替换私钥、bearer token、npm token、GitLab `glpat-`、Slack `xox*`、AWS access key、Google API key、JWT 与凭据赋值 |
...
| [discoveryTokenStore.ts](../src/security/discoveryTokenStore.ts) | 服务端 discovery token 存储：`issueToken`/`consumeToken`（单次使用）/`peekToken`/`revokeToken`，纯内存、10 分钟 TTL、活动上限 1024 |
| [workspaceRootGuard.ts](../src/security/workspaceRootGuard.ts) | `validateWorkspaceRoot` 校验工作区根目录合法性（被桌面应用动态 import 复用），拒绝盘符根、用户主目录、Desktop、Downloads、Documents |
...
| [processSecurity.ts](../src/runner/processSecurity.ts) | `SecureProcessLogCapture` 有界收集 + 写前脱敏 + `appendFileSync`；`buildChildEnvironment` 构造最小环境；`resolveTrustedExecutable` / `resolvePackageManagerInvocation` 受信解析 |
| [taskStatusStore.ts](../src/runner/taskStatusStore.ts) | 任务状态原子存储：`claimPendingTask` 同目录硬链接锁 + 原子 JSON 替换实现单次领取；`mutateTaskStatus` 共享状态锁，与 `cancelTask` 串行化 |
| [taskRuntime.ts](../src/runner/taskRuntime.ts) | `runtime.json` 浅合并读写，含 PID 重用与孤儿任务检测字段 |
| [taskProgress.ts](../src/runner/taskProgress.ts) | `progress.md` 生成，6 阶段标记（`[x]`/`[>]`/`[ ]`） |
...
| [diagnostics/schemaDriftCheck.ts](../src/tools/diagnostics/schemaDriftCheck.ts) | Schema 漂移自检，供 doctor 消费 |
| [tasks/createTask.ts](../src/tools/tasks/createTask.ts) | `createTask` 任务创建：支持 saved/inline/template 来源及 `assess_only` 风险预评估；定义 `TaskStatus` 与 `TaskPhase` 类型 |
| [tasks/runTaskLoop.ts](../src/tools/tasks/runTaskLoop.ts) | **安全编排入口**：组合 create_task → wait_for_task → safe summaries → audit_task，支持 worktree 隔离和 Direct 验证 |
| [tasks/taskLineage.ts](../src/tools/tasks/taskLineage.ts) | 链路记录，写入 `.patchwarden/lineages/<lineage_id>/` |
| [tasks/evidencePack.ts](../src/tools/tasks/evidencePack.ts) | 证据包导出，不含 stdout/stderr、完整 diff 或敏感内容 |
| [diagnostics/auditTask.ts](../src/tools/diagnostics/auditTask.ts) | 独立审计，区分 confirmed failures、possible false positives 与 manual verification；文档扫描最多 200 个 Markdown、4 MiB 总内容，证据读取有界且超限产生 warning |
| [diagnostics/safeViews.ts](../src/tools/diagnostics/safeViews.ts) | `safe_*` 系列有界摘要；其中 audit/finalize 包装器会写审计或会话状态，风险元数据为 `workspace_write` |
| [diagnostics/healthCheck.ts](../src/tools/diagnostics/healthCheck.ts) | 返回 MCP catalog、watcher、workspace 与 agents 状态 |
| [tasks/waitForTask.ts](../src/tools/tasks/waitForTask.ts) | 长轮询任务，返回 continuation 或终态 acceptance 证据 |
| [workspace/androidDoctor.ts](../src/tools/workspace/androidDoctor.ts) | 只读诊断 Android 构建环境 |
| [release/releaseMode.ts](../src/tools/release/releaseMode.ts) | 发布模式四件套与项目策略入口 |
| [discovery/explainTool.ts](../src/tools/discovery/explainTool.ts) | 展开单个工具详情并做 schema drift 检测 |
| [workspace/syncFile.ts](../src/tools/workspace/syncFile.ts) | Direct 会话仓库内复制文件，统一校验会话状态、真实路径、UTF-8/大小、敏感路径/内容与 sha256，并在写入前复核源/目标未变化 |
| [goals/goalSubgoalTask.ts](../src/tools/goals/goalSubgoalTask.ts) | 在 Goal mutation lock 内创建 subgoal 并关联新任务；以 Goal 保存的 `repo_path` 为权威，拒绝调用方仓库不一致，隔离 worktree 也从该仓库创建 |
| [workspace/mergeWorktree.ts](../src/tools/workspace/mergeWorktree.ts) | 合并隔离 worktree 的 branch 回主工作区 |
| [workspace/discardWorktree.ts](../src/tools/workspace/discardWorktree.ts) | 丢弃隔离 worktree 并归档状态 |
| [tasks/runVerification.ts](../src/tools/tasks/runVerification.ts) | Direct 会话跑单条白名单验证命令 |
| [direct/runDirectVerificationBundle.ts](../src/tools/direct/runDirectVerificationBundle.ts) | Direct 会话批量跑多个白名单验证命令并返回有界状态 |
...
v1.5.1 引入的按业务领域分组的工具分派层，将原先 `registry.ts` 中庞大的 switch 拆分为 5 个 handler map。v1.6.1 加入 `validation.ts` 提供 `unknown` 输入的类型守卫辅助。
...
| [validation.ts](../src/tools/dispatch/validation.ts) | v1.6.1 新增：从 `unknown` 提取 string/number/boolean/array 的类型守卫；dispatch 层用它取代 `as any` 断言 |
...
v0.6.0 引入的 ChatGPT 直接编辑模式。v1.6.1 把 patch/sync/verify/finalize/audit 全部纳入同一 session mutation lock 串行化。
...
| [directGuards.ts](../src/direct/directGuards.ts) | Direct 守卫：`guardDirectSessionActive`/`guardDirectSessionFinalized`/`guardDirectPath`/`guardDirectReadPath`/`guardDirectWritePath`/`guardDirectPatchSize`/`isBinaryFile`；拒绝 junction/symlink 与 NTFS ADS |
| [directPatch.ts](../src/direct/directPatch.ts) | JSON patch 应用：支持 `replace_exact`/`insert_before`/`insert_after`/`replace_whole_file`；同一次读取校验 `expected_sha256`、UTF-8、文件大小和敏感内容，目标同目录写随机临时文件 + `renameSync` 原子替换并保留原权限；失败时清理临时文件 |
...
v1.5.1 将原 `controlCenter.ts` 拆分为聚焦的路由/中间件/运行时/共享模块，保持入口和 HTTP 行为不变。v1.6.1 加入 `routeTable.ts` 声明式路由表。
...
shared.ts (配置 bootstrap / 路径常量 / controlToken / HTTP helper)
  ...
  ├─ routeTable.ts (声明式路由表)
  ├─ middleware/
  │   ├─ auth.ts   (control token 校验 + 回环 Host)
  │   └─ static.ts (ui/ 静态文件服务)
  └─ routes/
...
| 文件 | 职责 |
| --- | --- |
| [server.ts](../src/control/server.ts) | HTTP 服务创建、回环 Host/DNS rebinding 前置校验、防嵌入响应头（CSP `frame-ancestors 'none'` + `X-Frame-Options: DENY`）、`handleRequest` 路由分发、POST control-token 校验、bootstrap/shutdown 生命周期 |
...
| [runtime.ts](../src/control/runtime.ts) | `probeHealthStatus`/`probeRuntimeHealth` 健康探测；`classifyStaleTask`/`augmentTaskWithStale` 四条 stale 规则；`writeStatusFile`/`recordEvent`/`readEvents`（在跨进程文件锁内 append + trim + 原子替换）；`isValidTaskId`/`isValidDirectSessionId` 校验；`readHiddenStaleIds`/`readHiddenDirectSessionIds` 隐藏 ID |
| [routeTable.ts](../src/control/routeTable.ts) | 声明式路由表：按 method + pattern（声明顺序优先）匹配，specific pattern 优先于 generic `:id` |
| [middleware/auth.ts](../src/control/middleware/auth.ts) | `checkControlToken` 校验 `x-patchwarden-control-token`，使用 `crypto.timingSafeEqual` 并先校验 Buffer 长度；复用 `security/loopbackHost.ts` 拒绝非回环 Host |
...
| [routes/sessions.ts](../src/control/routes/sessions.ts) | Direct session 列表/详情/安全摘要/finalize/audit/hide；统一调用 `isValidDirectSessionId` 白名单校验 |
...
| [routes/tasks.ts](../src/control/routes/tasks.ts) | 只读任务查询：列表过滤/stale 任务/任务详情/safe 视图；统一调用 `reconstructTaskEntry` helper |
| [routes/workspace.ts](../src/control/routes/workspace.ts) | 工作区列表、repo 列表（含 package.json 元数据）、单 repo `git status --short`；只拒绝值恰为 `..` 的路径段 |

### 4.10 Desktop 子包（desktop/）

v1.6.0 引入的 Electron 桌面应用，作为 Control Center 的 Windows GUI 外壳。v1.6.1 把 IPC、JSON、model-discovery、tunnel-process 数据全部从 `unknown` 校验。
...
| [src/assessments/assessmentStore.ts](../src/assessments/assessmentStore.ts) | Assessment 记录存储与 freshness 校验；v1.6.1 加入跨进程锁串行化 freshness 校验与读取 |
| [src/utils/atomicFile.ts](../src/utils/atomicFile.ts) | `atomicWriteFileSync`/`atomicWriteJsonFileSync`：`.tmp-{pid}` 临时文件 + `renameSync` 原子替换 |
| [src/utils/boundedFile.ts](../src/utils/boundedFile.ts) | 有界读取：`readBoundedFileHead`/`readBoundedFileTail`/`readBoundedFileSlice` |
| [src/utils/lockedJsonFile.ts](../src/utils/lockedJsonFile.ts) | v1.6.1 核心：`withFileLockSync`/`mutateLockedJsonFileSync`，`O_EXCL` 创建 + PID 探活 + stale 接管 + 锁内 append/trim/原子替换 |
| [src/utils/platform.ts](../src/utils/platform.ts) | `isSamePath`/`isPathChildOf` 平台感知比较（Windows 忽略大小写） |
| [src/utils/stableJson.ts](../src/utils/stableJson.ts) | 唯一 `stableJsonStringify`；catalog 与 registry 均导入该实现 |
| [src/watcherStatus.ts](../src/watcherStatus.ts) | `readWatcherStatus` 心跳读取 + task heartbeat fallback + watcher ownership 检测；将 degraded 视为不可用 |
...
function getAssessmentsDir(config: PatchWardenConfig): string;
...
```typescript
// src/utils/lockedJsonFile.ts
function withFileLockSync<T>(lockPath: string, fn: () => T): T;
function mutateLockedJsonFileSync<T>(
  filePath: string,
  mutation: (current: T) => { next: T; result?: unknown },
  options?: { lockPath?: string; initialValue?: T }
): unknown;

// src/utils/atomicFile.ts
function atomicWriteFileSync(path: string, data: string | Buffer): void;
function atomicWriteJsonFileSync(path: string, data: unknown): void;
```
...
// 子阶段：prepareTask / executeAgent / runVerification / collectArtifacts / finalizeTask

// src/runner/watch.ts（脚本入口，部分导出供测试）
function acquireWatcherLock(lockFilePath?: string): void;  // 单实例排他锁 + stale PID 接管
function releaseWatcherLock(lockFilePath?, instanceId?, pid?): boolean;
function createNonOverlappingRunner(operation, onSkipped?): () => Promise<boolean>;
// tick() 每 4 秒轮询 pending 任务，pre-flight 校验后调用 runTask
...
// Windows 下使用小写路径键比较 before/after 快照、tracked/ignored/dirty
```
...
// dispatchMap 由 buildDispatchMap() 在模块加载时构造（配置无关）
...
// src/tools/dispatch/validation.ts （v1.6.1）
...
| `workspace-before.json` | 任务前工作区快照（外部脏文件基线） |
...
| `stdout.log` / `stderr.log` | Agent 与验证子进程的有界脱敏输出 |

资源边界也适用于读取和持续日志：普通任务产物受 `maxReadFileBytes` 限制，diff/summary/log tail 使用有界前缀或尾部读取；`audit_task` 的 Markdown 扫描上限为 200 个文件、4 MiB 总内容。`invocation.log` 与 `reconcile.log` 在跨进程锁内有界追加，达到上限后保留最近内容并写入截断标记，避免无界内存和磁盘增长。
...
index.ts / httpServer.ts
   ...
   runner/watch.ts ──▶ runner/runTask.ts ──▶ runner/changeCapture.ts
                                          ...
                                          ├── runner/processSecurity.ts
                                          └── runner/taskStatusStore.ts (锁内 mutate)

controlCenter.ts ──▶ control/server.ts ──┬── control/shared.ts
                                          ├── control/runtime.ts
                                          ├── control/routeTable.ts
                                          ├── control/middleware/{auth,static}.ts
                                          └── control/routes/*.ts (10 个领域)

desktop/main.ts ──▶ desktop/{agent-adapters,model-discovery,config-store,
                                tunnel-provisioner,runtime-settings,runtime-root,
                                backend-probe,backend-lifecycle,child-environment}.ts
                  └─ utilityProcess.fork ──▶ dist/controlCenter.js (主仓)

utils/lockedJsonFile.ts ◀── 所有跨进程 mutation 站点
utils/atomicFile.ts      ◀── 所有 .tmp-{pid} + rename 站点
utils/platform.ts        ◀── runtimeGuard / pathGuard / config
```
...
- **`runTask`** → `claimPendingTask`（taskStatusStore 锁内单次领取）
...
- **`registry.ts`** → 模块加载时 `buildDispatchMap()` 一次性合并 5 个 handler map（配置热重载后不刷新，需重连）
- **`control/server.ts`** → POST 路由统一前置 `checkControlToken`，route 文件本身不重复校验
- **`desktop/main.ts`** → `utilityProcess.fork` 拉起主仓 `dist/controlCenter.js`，通过 `127.0.0.1:8090` 通信
- **所有持久化状态 mutation** → 经 `utils/lockedJsonFile.ts` 跨进程锁 + `utils/atomicFile.ts` 原子替换
...
| `npm.cmd run desktop:preflight` | 桌面发布前预检（含 unpacked UI/单实例验收） |
| `npm.cmd run desktop:preflight:release` | 干净 checkout 的发布前预检（拒绝未提交路径） |
...
│  tools/dispatch/validation.ts (v1.6.1)          │  ← unknown → 类型守卫
...
│  runner/taskStatusStore.ts ← 锁内单次领取        │
├─────────────────────────────────────────────────┤
│  utils/lockedJsonFile.ts ← 跨进程 mutation 锁    │
│  utils/atomicFile.ts     ← .tmp-{pid} + rename   │
...
- 每个任务最多运行一次（无自动重试循环）；任务领取使用同目录硬链接锁 + 原子 JSON 替换
...
- HTTP Server 只绑定 `127.0.0.1`，并拒绝非回环 Host（防 DNS rebinding）
- Control Center POST 路由统一前置 `x-patchwarden-control-token` 校验，使用常量时间比较
- Windows DPAPI 加密 Tunnel 凭据，不写入仓库
- Desktop 应用 `contextIsolation` + `sandbox` + IPC sender 校验；IPC / JSON / model-discovery / tunnel-process 数据从 `unknown` 校验
...
6. Windows 下使用小写路径键比较 before/after 快照，避免大小写敏感目录碰撞
...
| 单元测试 | `npm.cmd run test:unit` | `src/test/unit/*.test.ts`（45+ 测试文件，按领域分组） |
...
- `runner/agent-invocation.test.ts`：原生 OpenCode、已知 npm shim、未知 wrapper 拒绝
- Desktop 测试覆盖 agent-detection / backend-lifecycle / backend-probe / child-environment / config-store / model-discovery / runtime-root / runtime-settings / tunnel-provisioner / ui-contract
...
输出到 `release/desktop/`：NSIS 安装包 + 免安装 ZIP + SHA256 校验和清单。首版不含自动更新或代码签名，Windows SmartScreen 可能提示未知发布者。`desktop:preflight` 会使用唯一目录完成 clean build、完整单测、桌面测试、npm 包面、Electron 目录包、仓库外 26-tool manifest 与隔离 unpacked UI/单实例验收，并生成 `preflight-report.json` / `preflight-report.md`。
...
> 本节在 v1.6.1 复核（2026-07-20）后重新整理：标记 "✅ 已修复" 的条目已按当前源码与仓库内对应测试核对，不再属于开放缺陷；标记 "🟡 开放" 的条目仍需处理；标记 "ℹ️ 非缺陷" 的条目经复核不构成问题。

### 11.1 跨平台缺陷（Windows + POSIX）

#### ✅ 11.1.1 `runtimeGuard.ts` 的 Windows 路径判断 — 已修复
...
#### ✅ 11.1.2 `changeCapture.ts` 的空设备路径 — 已修复
...
#### ✅ 11.1.4 Windows repo key 大小写匹配 — 已修复
...
#### ✅ 11.1.5 Windows Agent npm shim 无 shell 启动 — 已修复
...
#### ✅ 11.1.6 `postTaskCleanup.ts` 不再调用 `cmd rmdir` — 已修复
...
#### 🟡 11.2.1 `pathGuard.ts` 的 TOCTOU 竞争窗口 — 开放（HIGH）

- **文件**：[src/security/pathGuard.ts](../src/security/pathGuard.ts)
...
#### ✅ 11.2.2 `sensitiveGuard.ts` token/credentials 误匹配 — 已修复
...
#### ✅ 11.2.5 `contentRedaction.ts` 常见 token 格式覆盖 — 已修复
...
#### ✅ 11.2.6 `directPatch.ts` 原子替换 — 已修复
...
#### ✅ 11.2.7 `discoveryTokenStore.ts` 有界存储 — 已修复
...
#### ✅ 11.2.8 Control token 常量时间比较 — 已修复
...
#### ℹ️ 11.3.1 `release_prepare`/`release_cleanup` 的同步调用 — 非缺陷
...
#### ℹ️ 11.3.4 `watch.ts` 保持轮询定时器引用 — 非缺陷
...
#### ✅ 11.3.5 `simpleProcess.ts` 追加写入 — 已修复
...
#### 🟡 11.3.6 `logging.ts` fatal handler 立即退出 — 开放（LOW）
...
#### 🟡 11.3.7 best-effort cleanup 仍有静默 catch — 开放（LOW）
...
#### 🟡 11.4.2 `runTask.ts` 仍承担较多职责 — 开放（MEDIUM）
...
#### ℹ️ 11.4.3 `dispatchMap` 为配置无关 handler map — 非缺陷
...
#### 🟡 11.4.4 `any` 仍集中在测试夹具和少量 dispatch 转换 — 开放（MEDIUM，v1.6.1 部分收敛）

- **现状**：v1.6.1 引入 [src/tools/dispatch/validation.ts](../src/tools/dispatch/validation.ts)，把 dispatch 层的输入从 `as any` 断言迁到 `asString`/`asNumber`/`asBoolean`/`asStringArray`/`asRecord` 类型守卫；Desktop IPC、JSON、model-discovery、tunnel-process 数据也改为从 `unknown` 校验。剩余显著使用主要集中在大型 smoke harness 与少量 fixture。
- **建议**：优先为 MCP 输入和 smoke fixture 定义共享类型，逐步改为 `unknown` + 类型守卫；不要用一次性全局替换破坏现有测试意图。

#### 🟡 11.4.5 `coreDispatch.ts`/`directDispatch.ts`/`goalDispatch.ts`/`releaseDispatch.ts` 残余 `as any` — 开放（MEDIUM）

- **文件**：`src/tools/dispatch/*.ts` 多处
- **问题**：`template as any`、`file as "stdout" | "stderr" | ...`、`target_stage as any`、`operations as any` 等类型断言仍存在，依赖底层工具函数自行校验。v1.6.1 的 `validation.ts` 仅覆盖了部分高频入口。
- **修复建议**：在 dispatch 层做参数 schema 校验，或在底层加显式校验。

#### ✅ 11.4.6 共享稳定 JSON 实现 — 已修复

- **现状**：[src/utils/stableJson.ts](../src/utils/stableJson.ts) 提供唯一 `stableJsonStringify`；catalog 与 registry 均导入该实现。

#### 🟡 11.4.7 Control URL 解码策略仍未完全统一 — 开放（LOW）
...
#### ✅ 11.5.2 Watcher 重复 tick 失败升级 — 已修复
...
#### ✅ 11.5.3 Diagnostic dispatch 循环依赖 — 已修复
...
#### 🟡 11.6.1 核心模块测试覆盖 — 部分修复（MEDIUM）

已覆盖的定向测试见 [10.1 节](#101-测试体系)。剩余差距主要是 `runTask.ts` 全生命周期的细粒度单元覆盖和 `control/server.ts` HTTP 路由级覆盖；现有 lifecycle/control smoke 不能完全替代单元测试。

#### ℹ️ 11.6.2 工具级覆盖清单 — 旧清单失效

- 旧版按 `src/tools/*.ts` 平铺路径列出的"未覆盖文件"已因工具分域重排和新增测试失效。当前仍应关注的是跨工具生命周期、Control HTTP 边界和真实 CLI 组合覆盖，而不是维护容易漂移的静态文件名清单。

### 11.7 文档漂移

#### ✅ 11.7.1 CODE_WIKI 关键元数据漂移 — 已修复（v1.6.1 复核）
...
本文现已覆盖 `src/tools/dispatch/` 拆分、`src/control/` 路由拆分、`routeTable.ts`、`dispatch/validation.ts`、`utils/lockedJsonFile.ts`、Desktop 子包、模型发现、Agent 适配器、`androidDoctor`、`releaseMode`、`schemaDriftCheck`、`toolUsageStats`、`syncFile`、`goalSubgoalTask`、`runDirectVerificationBundle` 等。

#### ✅ 11.7.3 README 与 Direct Profile 工具数 — 已修复

- v1.6.1 时 `README.md`、`README.en.md` 与
  [src/tools/catalog/toolCatalog.ts](../src/tools/catalog/toolCatalog.ts) 已同步为
  14 个 `chatgpt_direct` 工具。v1.7.0 新增受限文件操作后，当前 catalog 与
  Tunnel 示例同步为 18 个工具。

### 11.8 配置/校验差距

#### ✅ 11.8.1 Direct 数值配置范围 — 已修复

- **现状**：[src/config.ts](../src/config.ts) 要求 `directSessionTtlSeconds` 为 60–86400 的整数，并要求 `directMaxPatchBytes`、`directMaxFileBytes` 为正整数。

#### ✅ 11.8.2 `control/routes/sessions.ts` sessionId 校验 — 已修复
...
#### ✅ 11.9.1 多 watcher 与任务重复领取（见 11.3.3） — 已修复

#### ✅ 11.9.2 Direct session 原子 mutation — 已修复
...
#### ✅ 11.9.3 Control event append/trim 竞争 — 已修复
...
#### ✅ 11.9.4 Hidden ID 文件类型校验 — 已修复

- **现状**：[src/control/runtime.ts](../src/control/runtime.ts) 通过 `readStoredStringArray` 校验 `Array.isArray`，过滤非字符串/空字符串并去重；写入使用原子 JSON 替换。

#### ✅ 11.9.5 Goal mutation 与 worktree lifecycle 串行化 — 已修复（v1.6.1）

- **现状**：[src/goal/goalStore.ts](../src/goal/goalStore.ts) 与 [src/goal/worktreeManager.ts](../src/goal/worktreeManager.ts) 共享仓库级 lifecycle lock；create/merge/discard 串行化，跨进程孤儿锁由后续操作自动接管。

### 11.10 依赖卫生

#### ℹ️ 11.10.1 主包保持单一直接运行时依赖 — 非缺陷

主仓 `package.json` 的直接运行时依赖只有 `@modelcontextprotocol/sdk`（`^1.29.0`）。这不是"零运行时依赖"；准确说法是"单一直接运行时依赖"。SDK 的传递依赖仍由 lockfile 管理。

#### ✅ 11.10.2 Node 类型与运行时基线对齐 — 已修复

- **现状**：主包使用 `@types/node ^18.19.0` 对齐 `engines.node >=18.0.0`；Electron 桌面子包独立使用 `@types/node ^24.0.0` 对齐其嵌入的 Node 运行时。

### 11.11 优先级修复建议汇总

**已修复并从开放待办移除**：11.1.1–11.1.6、11.2.2、11.2.5–11.2.8、11.3.2、11.3.3、11.3.5、11.4.1、11.4.6、11.4.8、11.5.2、11.5.3、11.7.1–11.7.3、11.8.1–11.8.3、11.9.1–11.9.5、11.10.2。11.2.3、11.2.4、11.3.1、11.3.4、11.4.3、11.6.2、11.10.1 经复核不构成缺陷。
...
3. [11.4.4] 持续把剩余 MCP 输入与 smoke fixture 的 `any` 迁移为共享类型/类型守卫
4. [11.4.5] dispatch 层残余 `as any` 收敛
5. [11.6.1] 补齐 `runTask.ts` 生命周期与 Control HTTP 路由级单元测试
6. [11.5.1] 模块级可变状态逐步收敛（带 reset helper 或显式 lifecycle）

**长期改进（LOW，可维护性）**：

7. [11.3.6] fatal handler 增加有界 flush，避免异步遥测丢失
8. [11.3.7] best-effort cleanup 增加有界 debug 计数
9. [11.4.7] 统一 strict/fallback 两类 URL 解码策略
...
11. **跨进程串行化（v1.6.1）**：所有持久化状态变更通过 `withFileLockSync` + `mutateLockedJsonFileSync` + 原子 `.tmp-{pid}` rename，保证 crash 后磁盘状态非旧即新
