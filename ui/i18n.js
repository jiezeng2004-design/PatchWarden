(function () {
  ...
  var dictionaries = {
    "zh-CN": {
      ...
      "settings.localAgentsHelp": "Codex 和 Claude 重读本地配置；OpenCode 等具备有界列表命令的 Agent 仅在点击刷新时查询 CLI。PatchWarden 不返回或持久化配置中的凭据值。",
      ...
      "settings.agentCliState": "CLI",
      "settings.modelConfigSourceState": "配置来源",
      "settings.modelConfigSourceMissing": "未读取到本地配置",
      "settings.modelCatalogState": "模型目录",
      "settings.modelEffectiveState": "当前生效模型",
      "settings.modelProviderState": "Provider 验证",
      ...
      "settings.reloadingModels": "正在重读 {agent} 本地模型配置…",
      "settings.modelsRefreshed": "已发现 {count} 个模型",
      "settings.modelsReloaded": "已从本地配置重读 {count} 个模型",
      "settings.modelsDiscovered": "已自动读取 {count} 个模型",
      "settings.reloadModels": "重新读取本地模型目录",
      "settings.modelCatalogCached": "模型目录已缓存，更新于 {time}",
      "settings.modelCatalogEmpty": "已检查，未发现模型",
      "settings.modelCatalogConfigured": "已读取 Agent 配置中的模型",
      "settings.modelCatalogCount": "目录中有 {count} 个模型",
      "settings.modelCatalogConfigOnly": "此 Agent 仅支持读取本地配置",
      "settings.modelCatalogNotChecked": "模型目录尚未检查",
      "settings.modelProviderNotChecked": "Provider 可用性尚未验证",
      "settings.modelProviderNotAvailable": "本轮未启用安全 Provider 验证",
      "settings.verifyModel": "验证当前模型",
      "settings.verifyingModel": "正在验证 {agent} 当前模型…",
      "settings.envAllowlistLabel": "运行时环境变量名称白名单",
      "settings.envAllowlistPlaceholder": "NAME, OTHER_NAME",
      "settings.envAllowlistState": "仅保存名称；当前 {present}/{total} 个名称在环境中存在",
      "settings.envAllowlistInvalid": "环境变量白名单只能包含逗号分隔的 NAME，不能填写 NAME=value",
      "settings.modelReason.ok": "模型验证通过",
      "settings.modelReason.not_checked": "尚未检查",
      "settings.modelReason.catalog_empty": "模型目录为空",
      "settings.modelReason.agent_unavailable": "Agent CLI 当前不可用",
      "settings.modelReason.refresh_unsupported": "此 Agent 不支持自动枚举模型",
      "settings.modelReason.refresh_timed_out": "模型目录刷新超时",
      "settings.modelReason.refresh_failed": "模型目录刷新失败",
      "settings.modelReason.unsupported_safe_probe": "当前 CLI 不支持安全的模型验证",
      "settings.modelReason.probe_timed_out": "模型验证超时",
      "settings.modelReason.authentication_failed": "Agent 登录或认证不可用",
      "settings.modelReason.model_rejected": "Provider 拒绝或不识别该模型",
      "settings.modelReason.probe_failed": "模型验证失败",
      "settings.modelReason.probe_output_invalid": "模型响应未通过验证",
      "settings.modelReason.invalid_model": "模型 ID 无效",
      "settings.modelReason.save_failed": "Agent 设置保存失败",
      "settings.modelReason.unexpected_result": "操作未能完成，请重新检测后重试",
      "settings.theme": "主题",
      ...
      "settings.savedRestart": "设置已保存，但运行中的 Core 尚未载入；请重启 Core 后再执行新任务",
      "settings.savedReload": "设置已保存并已由运行中的 Core 载入；ChatGPT 会话模型不受影响",
    ...
    en: {
      ...
      "settings.localAgentsHelp": "Codex and Claude reload local configuration; OpenCode and other Agents with bounded list commands query their CLI only when you click refresh. PatchWarden never returns or persists credential values from those settings.",
      ...
      "settings.agentCliState": "CLI",
      "settings.modelConfigSourceState": "Configuration source",
      "settings.modelConfigSourceMissing": "No local configuration was read",
      "settings.modelCatalogState": "Model catalog",
      "settings.modelEffectiveState": "Current effective model",
      "settings.modelProviderState": "Provider check",
      ...
      "settings.reloadingModels": "Reloading local {agent} model configuration…",
      "settings.modelsRefreshed": "Found {count} models",
      "settings.modelsReloaded": "Reloaded {count} models from local configuration",
      "settings.modelsDiscovered": "Automatically read {count} models",
      "settings.reloadModels": "Reload local model catalog",
      "settings.modelCatalogCached": "Model catalog cached at {time}",
      "settings.modelCatalogEmpty": "Checked; no models were found",
      "settings.modelCatalogConfigured": "Models read from Agent configuration",
      "settings.modelCatalogCount": "Catalog contains {count} models",
      "settings.modelCatalogConfigOnly": "This Agent supports local configuration discovery only",
      "settings.modelCatalogNotChecked": "Model catalog has not been checked",
      "settings.modelProviderNotChecked": "Provider availability has not been verified",
      "settings.modelProviderNotAvailable": "Safe Provider verification is not enabled in this release",
      "settings.verifyModel": "Verify current model",
      "settings.verifyingModel": "Verifying the current {agent} model…",
      "settings.envAllowlistLabel": "Runtime environment variable name allow-list",
      "settings.envAllowlistPlaceholder": "NAME, OTHER_NAME",
      "settings.envAllowlistState": "Names only; {present} of {total} are present in the environment",
      "settings.envAllowlistInvalid": "The allow-list accepts comma-separated NAME entries only, not NAME=value",
      "settings.modelReason.ok": "Model verification passed",
      "settings.modelReason.not_checked": "Not checked",
      "settings.modelReason.catalog_empty": "The model catalog is empty",
      "settings.modelReason.agent_unavailable": "The Agent CLI is unavailable",
      "settings.modelReason.refresh_unsupported": "This Agent cannot enumerate models automatically",
      "settings.modelReason.refresh_timed_out": "Model catalog refresh timed out",
      "settings.modelReason.refresh_failed": "Model catalog refresh failed",
      "settings.modelReason.unsupported_safe_probe": "This CLI does not support a safe model probe",
      "settings.modelReason.probe_timed_out": "Model verification timed out",
      "settings.modelReason.authentication_failed": "Agent sign-in or authentication is unavailable",
      "settings.modelReason.model_rejected": "The Provider rejected or does not recognize this model",
      "settings.modelReason.probe_failed": "Model verification failed",
      "settings.modelReason.probe_output_invalid": "The model response could not be verified",
      "settings.modelReason.invalid_model": "The model ID is invalid",
      "settings.modelReason.save_failed": "Agent settings could not be saved",
      "settings.modelReason.unexpected_result": "The operation could not be completed; detect again and retry",
      ...
      "settings.savedRestart": "Settings saved, but the running Core has not loaded them. Restart Core before creating a new task.",
      "settings.savedReload": "Settings saved and loaded by the running Core. The ChatGPT conversation model is unchanged.",
