...
export async function createSubgoalTask(input: CreateSubgoalTaskInput): Promise<CreateSubgoalTaskOutput> {
  ...
  return mutateGoalStatusAsync(input.goal_id, async (goalStatus) => {
  if (goalStatus.status !== "active") {
    throw new PatchWardenError(
      "goal_not_active",
      `Goal "${input.goal_id}" is ${goalStatus.status} and cannot accept new subgoals.`,
      "Create a new Goal Session for additional work.",
      true,
      { goal_id: input.goal_id, status: goalStatus.status },
    );
  }

  const goalRepoPath = guardWorkspacePath(goalStatus.repo_path, config.workspaceRoot);
  const requestedRepoPath = guardWorkspacePath(input.repo_path, config.workspaceRoot);
  const sameRepo = process.platform === "win32"
    ? resolve(goalRepoPath).toLowerCase() === resolve(requestedRepoPath).toLowerCase()
    : resolve(goalRepoPath) === resolve(requestedRepoPath);
  if (!sameRepo) {
    throw new PatchWardenError(
      "goal_repo_mismatch",
      `Subgoal repo_path does not match Goal "${input.goal_id}".`,
      "Use the repo_path recorded on the Goal Session.",
      true,
  ...
  let repoPathForTask = goalRepoPath;
  let worktreeId: string | null = null;
  if (isolate) {
    const wt = createWorktree(input.goal_id, subgoalId, goalRepoPath);
  ...
  try {
    ...
    if (isolate && worktreeId) {
      try {
        discardWorktree(worktreeId, goalRepoPath);
