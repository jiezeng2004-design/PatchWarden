...
export function safeResult(taskId: string, options: SafeViewOptions = {}) {
  ...
  return redact({
    ...
    agent_failure_category: summary.agent_failure_category,
    failure_source: summary.failure_source,
    counts_against_agent: summary.counts_against_agent,
    fallback_eligible: summary.fallback_eligible,
    retryable: summary.retryable,
    completion_state: summary.completion_state,
    project_fact_validation: summary.project_fact_validation,
    framework_validation: summary.framework_validation,
    svg_xml_validation: summary.svg_xml_validation,
    acceptance_report: summary.acceptance_report,
    ...
    information: limitStrings(summary.information, maxItems),
...
function auditToSafe(audit: AuditTaskOutput, maxItems: number) {
  ...
  return redact({
    ...
    agent_failure_category: audit.agent_failure_category,
    failure_source: audit.failure_source,
    counts_against_agent: audit.counts_against_agent,
    fallback_eligible: audit.fallback_eligible,
...
function directSessionToSafe(session: DirectSessionRecord, maxItems: number, view: "summary" | "finalize") {
  ...
  return redact({
    ...
    dependency_changes: limitClassified(artifacts?.artifact_hygiene.dependency_changes, maxItems),
    generated_changes: limitClassified(artifacts?.artifact_hygiene.generated_changes, maxItems),
    runtime_changes: limitClassified(artifacts?.artifact_hygiene.runtime_changes, maxItems),
    unexpected_changes: limitClassified(artifacts?.artifact_hygiene.unexpected_changes, maxItems),
...
function readJson(path: string, config: ReturnType<typeof getConfig>): Record<string, unknown> {
