...
const MAX_AUDIT_DOC_FILES = 200;
const MAX_AUDIT_DOC_FILE_BYTES = 512 * 1024;
const MAX_AUDIT_DOC_TOTAL_BYTES = 4 * 1024 * 1024;
const MAX_AUDIT_EVIDENCE_BYTES = 1024 * 1024;
...
function findMdFiles(
    dir: string,
    maxDepth = 3,
    maxFiles = MAX_AUDIT_DOC_FILES,
): { files: string[]; truncated: boolean } {
    const results: string[] = [];
    const pending = [{ path: dir, depth: maxDepth }];
    const maxDirectories = maxFiles * 4;
    let visitedDirectories = 0;
    let truncated = false;
    while (pending.length > 0 && results.length < maxFiles && visitedDirectories < maxDirectories) {
        const current = pending.pop()!;
        if (!existsSync(current.path) || current.depth <= 0) continue;
        ...
        try {
            ...
            while (results.length < maxFiles) {
                ...
                if (entry.isDirectory() && !entry.name.startsWith(".") && entry.name !== "node_modules") {
                    if (pending.length < maxDirectories) pending.push({ path: full, depth: current.depth - 1 });
                    else truncated = true;
...
function readAuditEvidence(path: string): { content: string; truncated: boolean } {
    ...
    if (!prefix.truncated) return { content: prefix.content, truncated: false };
    const tail = readTextFileTailLinesSync(path, 1000, half);
    return {
        content: `${prefix.content}\n[...audit evidence truncated...]\n${tail}`,
        truncated: true,
    };
}
...
export function auditTask(taskId: string): AuditTaskOutput {
    ...
    let markdownEnumerationTruncated = false;
    if (existsSync(docsDir)) {
        const markdown = findMdFiles(docsDir);
        docsToScan.push(...markdown.files);
        markdownEnumerationTruncated = markdown.truncated;
    }
    ...
    for (const docPath of docsToScan) {
        ...
        try {
            ...
            const bounded = readTextFilePrefixSync(
                docPath,
                Math.min(MAX_AUDIT_DOC_FILE_BYTES, remainingDocBytes),
            );
            content = bounded.content;
            remainingDocBytes -= Buffer.byteLength(content, "utf-8");
            documentScanTruncated ||= bounded.truncated;
        }
        catch {
            continue;
        }
    ...
    if (documentScanTruncated) {
        checks.push({
            name: "documentation_scan_bounded",
            result: "warn",
            detail: `Documentation scan reached its ${MAX_AUDIT_DOC_FILES}-file or ${MAX_AUDIT_DOC_TOTAL_BYTES}-byte safety budget.`,
        });
    ...
    if (hasTestLog) {
        ...
        try {
            const evidence = readAuditEvidence(testLogFile);
            logContent = evidence.content;
            if (evidence.truncated) addManualVerification("test.log exceeded the audit read budget; inspect the complete local artifact.");
        }
        catch {
            logContent = "";
        }
    ...
    if (hasResult) {
        try {
            const evidence = readAuditEvidence(resultFile);
            resultMdContent = evidence.content;
            if (evidence.truncated) addManualVerification("result.md exceeded the audit read budget; inspect the complete local artifact.");
        }
        catch {
    ...
    atomicWriteFileSync(join(taskDir, "independent-review.md"), reviewMd);
    ...
    if (taskStatus === "done_by_agent") {
        ...
        mutateTaskStatus(statusFile, (current) => {
            ...
            const next = { ...current, ...statusPatch, updated_at: new Date().toISOString() };
