...
import { resolveTrustedCommandLine, buildGitEnvironment, resolveTrustedExecutable } from "../../runner/processSecurity.js";
...
export function releasePrepare(input: ReleasePrepareInput): ReleaseModeResult {
  ...
  for (const command of commands) {
    ...
    try {
      const invocation = resolveTrustedCommandLine(command, repoPath);
      const execution = runSimpleProcessSync({
        command: invocation.command,
        args: invocation.argsPrefix,
        cwd: repoPath,
        timeoutMs,
      });
      if (execution.spawnError) throw new Error(execution.spawnError);
      if (execution.timedOut) throw new Error(`Command timed out after ${timeoutMs}ms`);
      if (execution.exitCode !== 0) {
        const status = execution.exitCode ?? -1;
        throw Object.assign(new Error(`Command exited with code ${status}`), { status });
      }
...
export function releaseCleanup(input: ReleaseCleanupInput): ReleaseModeResult {
  ...
  atomicWriteJsonFileSync(reportPath, report);
...
function isTrackedByGit(root: string, relPath: string): boolean {
  ...
  try {
    ...
    execFileSync(resolveTrustedExecutable("git", root, { pathValue: env.PATH }), ["ls-files", "--error-unmatch", "--", relPath], {
      cwd: root,
      stdio: "ignore",
      windowsHide: true,
      env,
