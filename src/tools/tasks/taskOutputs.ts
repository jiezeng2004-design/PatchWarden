...
export function getDiff(taskId: string): GetTaskFileOutput {
  ...
  const bounded = readTextFilePrefixSync(filePath, config.maxReadFileBytes);
  const size = bounded.totalBytes;
  const patchHead = bounded.content;
  const redaction = redactSensitiveContent(patchHead);
  const content = redaction.content;
  const diff: GetTaskFileOutput & { truncated: boolean } = {
    task_id: taskId,
  ...
  } as GetTaskFileOutput & { truncated: boolean };
  try {
...
export function getTaskLogTail(
  ...
  const tail = readTextFileTailLinesSync(filePath, maxLines);
...
function getTaskFileTail(
  ...
  const stat = statSync(filePath);
  const maxBytes = options.maxBytes > 0
    ? Math.min(options.maxBytes, config.maxReadFileBytes)
    : config.maxReadFileBytes;
  let content: string;
  if (options.tailLines > 0) {
    content = readTextFileTailLinesSync(
      filePath,
      Math.min(Math.max(1, options.tailLines), 1000),
      maxBytes,
    );
  } else {
    content = readTextFilePrefixSync(filePath, maxBytes).content;
  }
