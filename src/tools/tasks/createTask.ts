...
export interface CreateTaskInput {
  ...
  goal_id?: string;
  subgoal_id?: string;
  /** Internal provenance supplied by retry_task; not exposed in MCP schemas. */
  retry_metadata?: {
    retry_of: string;
    retry_count: number;
...
export async function createTask(input: CreateTaskInput): Promise<CreateTaskResult> {
  ...
  const retryMetadata = effectiveInput.retry_metadata;
  const safeAcceptance = redactSensitiveValue({
    goal: effectiveInput.goal || null,
  ...
  const statusData = {
    ...
    task_id: taskId,
    plan_id: planId,
    plan_source: retryMetadata?.plan_source ?? planSource,
    template: retryMetadata?.template ?? effectiveInput.template ?? null,
    change_policy: retryMetadata?.change_policy ?? changePolicy,
    ...(retryMetadata ? {
      retry_of: retryMetadata.retry_of,
    ...
    // v0.7.2: 验收标准绑定到任务创建
    goal: safeAcceptance.goal,
    scope: safeAcceptance.scope,
    forbidden: safeAcceptance.forbidden,
