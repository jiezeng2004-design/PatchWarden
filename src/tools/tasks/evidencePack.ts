...
import { getLastToolCatalogSnapshot } from "../catalog/toolCatalog.js";
...
export function exportTaskEvidencePack(input: { lineage_id: string; max_items?: number }): SafeEvidencePack {
  ...
  const attestationContent = buildAttestation(catalog, generatedAt, lineage.repo_path, config.workspaceRoot);
  ...
  atomicWriteJsonFileSync(filePaths.risk, redactSensitiveValue(riskContent).value);
  atomicWriteJsonFileSync(filePaths.verify, redactSensitiveValue(verifyContent).value);
  atomicWriteJsonFileSync(filePaths.diffstat, redactSensitiveValue(diffstatContent).value);
  atomicWriteJsonFileSync(filePaths.lineage, redactSensitiveValue(lineageSummary).value);
  atomicWriteJsonFileSync(filePaths.attestation, redactSensitiveValue(attestationContent).value);
  atomicWriteJsonFileSync(filePaths.redactions, redactSensitiveValue(redactionsContent).value);
  ...
  atomicWriteJsonFileSync(safePack.files.json, safePack);
  atomicWriteFileSync(safePack.files.markdown, buildEvidenceMarkdown(safePack));
  return safePack;
}
...
function readGitCommitShort(repoPath: string, workspaceRoot: string): string {
  try {
    const cwd = guardWorkspacePath(repoPath || ".", workspaceRoot);
    const env = buildGitEnvironment(cwd);
    const git = resolveTrustedExecutable("git", cwd, { pathValue: env.PATH });
...
function buildAttestation(
  ...
  return {
    ...
    commit: readGitCommitShort(repoPath, workspaceRoot),
