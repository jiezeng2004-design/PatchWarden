...
/**
 * Run a shell-free command and return its trimmed combined stdout/stderr output.
 ...
 */
function runCmd(command: string, args: readonly string[], cwd = process.cwd()): string {
  try {
...
export function diagnoseAndroidBuild(repoPath: string): AndroidBuildDiagnostic {
  ...
  // 1. java -version ------------------------------------------------------
  // `java -version` normally writes to stderr; runSimpleProcessSync captures both.
  const javaOutput = runCmd("java", ["-version"], repoPath);
