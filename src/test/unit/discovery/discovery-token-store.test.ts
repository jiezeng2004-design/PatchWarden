...
describe("discoveryTokenStore", () => {
  ...
  describe("issueToken", () => {
    ...
    it("签发新 token 时清理已过期且从未消费的记录", () => {
      const expired = issueToken(sampleInput({ ttlMs: -1000 }));
      const fresh = issueToken(sampleInput());
      assert.equal(peekToken(expired), null);
      assert.ok(peekToken(fresh));
      assert.equal(getActiveTokenCount(), 1);
    });

    it("限制活动 token 数量并淘汰最旧记录", () => {
      const oldest = issueToken(sampleInput());
      let newest = oldest;
      for (let index = 0; index < MAX_ACTIVE_DISCOVERY_TOKENS; index += 1) {
        newest = issueToken(sampleInput({ query: `query-${index}` }));
      }
      assert.equal(getActiveTokenCount(), MAX_ACTIVE_DISCOVERY_TOKENS);
      assert.equal(peekToken(oldest), null);
      assert.ok(peekToken(newest));
    });
  });
