...
import { addSubgoal, updateSubgoalStatus, type GoalStatus } from "../../../goal/goalStatus.js";
...
function assertInvalidGoalId(fn: () => unknown): void {
  assert.throws(fn, (err: unknown) => {
    assert.ok(err instanceof PatchWardenError);
    assert.equal(err.reason, "invalid_goal_id");
    return true;
  });
}
...
describe("goalStore", () => {
  ...
  describe("getGoalsDir / getGoalDir", () => {
    ...
    it("accepts a legal goal id at the maximum length", () => {
      const goalId = `goal_${"a".repeat(123)}`;
      assert.equal(goalId.length, 128);
      assert.equal(getGoalDir(goalId, tempDir), join(tempDir, ".patchwarden", "goals", goalId));
    });

    it("rejects traversal, absolute, separated, and overlong goal ids for every accessor", () => {
      const invalidIds = [
        "..",
        "../outside",
        "..\\outside",
        "goal/child",
        "goal\\child",
        join(tempDir, "absolute-goal"),
        `goal_${"a".repeat(124)}`,
      ];
      const placeholder = {} as GoalStatus;

      for (const goalId of invalidIds) {
        assertInvalidGoalId(() => getGoalDir(goalId, tempDir));
        assertInvalidGoalId(() => readGoal(goalId, tempDir));
        assertInvalidGoalId(() => readGoalStatus(goalId, tempDir));
        assertInvalidGoalId(() => writeGoalStatus(goalId, placeholder, tempDir));
      }
    });

    it("fails closed before traversal can read or overwrite a sibling status file", () => {
      const outsideDir = join(tempDir, ".patchwarden", "outside");
      const outsideStatus = join(outsideDir, "goal_status.json");
      const sentinel = '{"sentinel":true}\n';
      mkdirSync(outsideDir, { recursive: true });
      writeFileSync(outsideStatus, sentinel, "utf-8");

      assertInvalidGoalId(() => readGoalStatus("../outside", tempDir));
      assertInvalidGoalId(() => writeGoalStatus("../outside", {} as GoalStatus, tempDir));
      assert.equal(readFileSync(outsideStatus, "utf-8"), sentinel);
    });
  });
  ...
  describe("writeGoalStatus + readGoalStatus", () => {
    ...
    it("fails closed when a concurrent mutation already owns the Goal lock", () => {
      const { goal_id } = createGoal("repo", "Locked", "desc", tempDir);
      assert.throws(
        () => mutateGoalStatus(goal_id, (current) => {
          mutateGoalStatus(goal_id, () => ({ result: undefined }), tempDir);
          return { next: current, result: undefined };
        }, tempDir),
        (error: unknown) =>
          error instanceof PatchWardenError && error.reason === "goal_busy",
      );
      assert.equal(existsSync(join(getGoalDir(goal_id, tempDir), "goal_status.json.lock")), false);
    });
  });
});
