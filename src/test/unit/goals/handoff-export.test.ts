...
describe("handoffExport", () => {
  ...
  describe("exportHandoff", () => {
    ...
    it("拒绝 goal_id 路径逃逸且不在目标外写文件", () => {
      const goal = makeGoalStatus([]);
      assert.throws(
        () => exportHandoff("../outside", goal, tempDir),
        (error: unknown) => error instanceof Error && error.message.includes("Invalid goal id")
      );
      assert.equal(existsSync(resolve(tempDir, ".patchwarden", "outside", "handoff.md")), false);
    });

    it("原子替换 handoff 且不遗留临时文件", () => {
      const goal = makeGoalStatus([]);
      const result = exportHandoff("goal_atomic_001", goal, tempDir);
      exportHandoff("goal_atomic_001", { ...goal, title: "Updated goal" }, tempDir);

      assert.ok(readFileSync(result.handoff_path, "utf-8").includes("Updated goal"));
      assert.deepEqual(
        readdirSync(resolve(tempDir, ".patchwarden", "goals", "goal_atomic_001"))
