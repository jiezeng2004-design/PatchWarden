...
describe("goalReport", () => {
  ...
  it("原子替换报告且不遗留临时文件", () => {
    const goal = makeGoalStatus([]);
    seedGoalStatus("goal_atomic_001", goal, tempDir);

    const first = exportGoalReport("goal_atomic_001", { workspaceRoot: tempDir });
    exportGoalReport("goal_atomic_001", { workspaceRoot: tempDir });

    const reportDir = resolve(tempDir, ".patchwarden", "goals", "goal_atomic_001", "report");
    assert.ok(existsSync(first.files.report_md));
    assert.ok(existsSync(first.files.report_json));
    assert.deepEqual(readdirSync(reportDir).filter((name) => name.endsWith(".tmp")), []);
  });
});
