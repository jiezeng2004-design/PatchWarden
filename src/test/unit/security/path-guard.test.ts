...
describe("guardPath", () => {
  ...
  it("rejects a dangling symlink whose target would escape the workspace", { skip: process.platform === "win32" ? "Windows symlink permissions unstable" : undefined }, () => {
    const outside = mkdtempSync(join(tmpdir(), "pw-dangling-target-"));
    try {
      const linkPath = join(tempDir, "dangling-link");
      symlinkSync(join(outside, "future-secret.txt"), linkPath);
      assert.throws(
        () => guardPath("dangling-link", tempDir),
        PatchWardenError
      );
    } finally {
