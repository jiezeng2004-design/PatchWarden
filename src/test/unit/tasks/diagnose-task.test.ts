...
import { diagnoseTask } from "../../../tools/tasks/diagnoseTask.js";
...
describe("diagnoseTask", () => {
  ...
  it("bounds large diagnostic log reads to the requested tail", () => {
    buildTask({
      taskId: "task-large-logs-001",
      status: "running",
      phase: "executing_agent",
      heartbeatSecondsAgo: 10,
      childPid: ALIVE_PID,
      ...
      taskStartedSecondsAgo: 60,
      stdoutContent: [
        "old-diagnostic-sentinel",
        ...Array.from({ length: 100 }, (_, index) => `diagnostic-${index}`),
      ].join("\n"),
      stdoutSecondsAgo: 5,
      stderrContent: "stderr-final\n",
      stderrSecondsAgo: 5,
    });
    writeWatcherHeartbeat("watcher-A", 3);

    const result = diagnoseTask({ task_id: "task-large-logs-001", include_logs: true }, config);

    assert.ok(result.logs?.stdout_tail?.includes("diagnostic-99"));
    assert.ok(!result.logs?.stdout_tail?.includes("old-diagnostic-sentinel"));
  });
