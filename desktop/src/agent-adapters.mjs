...
const ADAPTER_BY_ID = new Map(AGENT_ADAPTERS.map((adapter) => [adapter.id, adapter]));
...
function isFile(path, fileExists = existsSync) {
  try { return fileExists(path); } catch { return false; }
}

function safeNativeCandidate(candidate, platform, fileExists) {
...
function resolveNpmEntry(adapter, shimDir, nodePath, fileExists = existsSync, readText = (path) => readFileSync(path, "utf8")) {
...
export function selectAgentLaunch(id, output, platform = process.platform, options = {}) {
...
export function buildAgentRegistration(id, detection, modelValue) {
  const adapter = getAgentAdapter(id);
...
async function detectOne(adapter, platform) {
  const lookup = platform === "win32" ? "where.exe" : "which";
  try {
    const [{ stdout }, nodeResult] = await Promise.all([
      execFileAsync(lookup, [adapter.id], { timeout: 5000, windowsHide: true, maxBuffer: 256 * 1024 }),
      platform === "win32" && adapter.npmPackage
        ? execFileAsync(lookup, ["node"], { timeout: 5000, windowsHide: true, maxBuffer: 64 * 1024 }).catch(() => ({ stdout: "" }))
...
export async function refreshAgentModels(id, detection) {
  const adapter = getAgentAdapter(id);
